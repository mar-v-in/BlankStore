// Code generated by Wire protocol buffer compiler, do not edit.
// Source file: proto/Purchase.proto
package com.google.play.proto;

import com.squareup.wire.Message;
import com.squareup.wire.ProtoField;

import static com.squareup.wire.Message.Datatype.INT32;
import static com.squareup.wire.Message.Datatype.STRING;

public final class PurchaseProductRequestProto extends Message {

  public static final Integer DEFAULT_PRODUCTTYPE = 0;
  public static final String DEFAULT_PRODUCTID = "";

  @ProtoField(tag = 1, type = INT32)
  public final Integer productType;

  @ProtoField(tag = 2, type = STRING)
  public final String productId;

  @ProtoField(tag = 3)
  public final SignatureHashProto signatureHash;

  public PurchaseProductRequestProto(Integer productType, String productId, SignatureHashProto signatureHash) {
    this.productType = productType;
    this.productId = productId;
    this.signatureHash = signatureHash;
  }

  private PurchaseProductRequestProto(Builder builder) {
    this(builder.productType, builder.productId, builder.signatureHash);
    setBuilder(builder);
  }

  @Override
  public boolean equals(Object other) {
    if (other == this) return true;
    if (!(other instanceof PurchaseProductRequestProto)) return false;
    PurchaseProductRequestProto o = (PurchaseProductRequestProto) other;
    return equals(productType, o.productType)
        && equals(productId, o.productId)
        && equals(signatureHash, o.signatureHash);
  }

  @Override
  public int hashCode() {
    int result = hashCode;
    if (result == 0) {
      result = productType != null ? productType.hashCode() : 0;
      result = result * 37 + (productId != null ? productId.hashCode() : 0);
      result = result * 37 + (signatureHash != null ? signatureHash.hashCode() : 0);
      hashCode = result;
    }
    return result;
  }

  public static final class Builder extends Message.Builder<PurchaseProductRequestProto> {

    public Integer productType;
    public String productId;
    public SignatureHashProto signatureHash;

    public Builder() {
    }

    public Builder(PurchaseProductRequestProto message) {
      super(message);
      if (message == null) return;
      this.productType = message.productType;
      this.productId = message.productId;
      this.signatureHash = message.signatureHash;
    }

    public Builder productType(Integer productType) {
      this.productType = productType;
      return this;
    }

    public Builder productId(String productId) {
      this.productId = productId;
      return this;
    }

    public Builder signatureHash(SignatureHashProto signatureHash) {
      this.signatureHash = signatureHash;
      return this;
    }

    @Override
    public PurchaseProductRequestProto build() {
      return new PurchaseProductRequestProto(this);
    }
  }
}
